...
export function ApplicationFilters({
        ...
        >
          ...
          {jobs.map((job) => (
            <MenuItem key={job.job_id} value={job.job_id}>
              <Typography variant="body2">{job.job_id} - {job.title}</Typography>
            </MenuItem>
          ))}
        </Select>
      </FormControl>
  ...
  const renderPostedBy = (
    <Stack spacing={1} sx={{ mb: 3 }}>
      <Typography variant="subtitle2">HR Filter</Typography>
      <TextField
        fullWidth
        size="small"
        value={filters.posted_by || ''}
        onChange={(event) => onFilters('posted_by', event.target.value)}
