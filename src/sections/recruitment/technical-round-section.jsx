...
export function TechnicalRoundSection({ filters, candidate }) {
  ...
  useEffect(() => {
    fetchInterviews('technical', filters);
  }, [fetchInterviews, filters]);

  // Filter out applications that already have scheduled technical interviews
  // Only show applications that passed telephone round (technical_pending)
  const eligibleApplications = applications.filter((app) => {
    ...
    return !hasScheduledInterview && app.status === 'technical_pending';
  });
    ...
    try {
      ...
      if (selectedApplication && scheduleDate) {
        ...
        const eventUrl = createGoogleCalendarEventUrl({
          title: `Interview with F13 Technologies - Technical Round`,
          description: `Technical interview round with F13 Technologies\nInterviewer: ${interviewer}\nAssigned By: ${assignedBy}`,
          startTime,
          endTime,
          ...
          setConfirmDialog({
            ...
            onConfirm: async () => {
              try {
                ...
                await fetchInterviews('technical', filters);
              } catch (error) {
                setConfirmDialog({
                  open: true,
                  title: 'Error',
...
TechnicalRoundSection.propTypes = {
